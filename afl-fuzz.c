...
EXP_ST u8 skip_deterministic = 1, /* Skip deterministic stages?       */
    ...
    use_splicing = 1,             /* Recombine input files?           */
...
EXP_ST u64 total_crashes, /* Total number of crashes          */
    //queued_paths_exp2,    /* queued_paths_exp2 for MAB        */
...
struct queue_entry
{
  ...
  u8 cal_failed,                  /* Calibration failed?              */
      ...
      arg_was_fuzzed,             /* Yuan-fuzz-cg use for arg_fuzz    */
      passed_det,                 /* Deterministic stages passed?     */
      has_new_cov,                /* Triggers new coverage?           */
      var_behavior,               /* Variable behavior?               */
      favored,                    /* Currently favored?               */
      fs_redundant,               /* Marked as redundant in the fs?   */
      decayed;                    /* MAB decay use */
  ...
  struct queue_entry *next, /* Next element, if any             */
      ...
      *arg_gen_parent;       /* if not generated by arg_gen -> self */
};
...
static void mab_decay()
{
  ...
  while (tmp_queue_cur)
  {
    tmp_queue_cur->arg_gen_parent->decayed = 0;
    tmp_queue_cur = tmp_queue_cur->next;
  }
}
...
#define MAB_ADD_CONST (2 << 20)
  ...
  if(use_mab && queue_cur)
  {
    ...
    q->total_splice_gen_path = (queue_cur->total_splice_gen_path + 1) / 2;
    if(strcmp(stage_name,"havoc")==0)
    {
      q->arg_gen_parent->total_arg_gen_exec_100us = (queue_cur->arg_gen_parent->total_arg_gen_exec_100us + 1)/2;
      q->arg_gen_parent->total_arg_gen_path += (queue_cur->arg_gen_parent->total_arg_gen_path + 1)/2;
      q->total_havoc_gen_path += MAB_ADD_CONST / 2;
    }
    else if(strncmp(stage_name,"splice",6)==0)
    {
      q->arg_gen_parent->total_arg_gen_exec_100us = (queue_cur->arg_gen_parent->total_arg_gen_exec_100us + 1)/2;
      q->arg_gen_parent->total_arg_gen_path += (queue_cur->arg_gen_parent->total_arg_gen_path + 1)/2;
      q->total_splice_gen_path += MAB_ADD_CONST / 2;
    }
  }
  ...
  if(use_mab && strncmp(stage_name,"sync",4)==0)
  {
    ...
    q->total_splice_exec_100us = MAB_ADD_CONST;
    q->total_splice_gen_path = MAB_ADD_CONST;
    q->arg_gen_parent->total_arg_gen_exec_100us = MAB_ADD_CONST;
    q->arg_gen_parent->total_arg_gen_path = MAB_ADD_CONST;
  }
...
abort_calibration:
  ...
  // yuan-fuzz-cg
  // argv_no_forkserver = temp_argv_no_forkserver;
...
EXP_ST u8 argv_common_fuzz_stuff(char **argv, u8 *out_buf, u32 len, int argv_array[])
{
  ...
  /* use exec to run */ // yuan-fuzz-cg -> more timeout for argv run because no fork server // 先拿掉
  fault = run_target(argv, exec_tmout);
...
static char **argv_fuzz_one(char **argv)
{
  ...
  stage_short = "arg1";
  stage_max = parameter_total_count + parameter_count;
  stage_name = "arg_gen_rand";
  stage_cur = 0;
  char **new_argv;
  ...
  if(queue_cur->arg_was_fuzzed || !use_cg)
    goto argv_abandon_entry;

  stage_short = "arg2";
  stage_max = parameter_total_count + parameter_count;
  stage_name = "arg_gen_det";
  stage_cur = 0;
  // only flip one arg at one time
  for (int i = 0; i < parameter_count;i++)
  {
    for (int j = 0; j < parameter[i].count + 1; j++)
    {
      stage_cur++;
      if(queue_cur->argv[i] == j || (parameter[i].must && j==0))continue;

      new_argv = (char **)ck_alloc(sizeof(char *) * 200);
      memset(new_argv, 0, sizeof(char *) * 200);
      int orig = queue_cur->argv[i];
      queue_cur->argv[i] = j;
...
static void sync_fuzzers(char **argv)
{
  ...
  while ((sd_ent = readdir(sd)))
  {
    ...
    if(argv_fuzz_flag)reset_forkserv_argvs(argv);
    while ((qd_ent = readdir(qd)))
    {
      ...
      if (st.st_size && st.st_size <= MAX_FILE)
      {
        ...
        if(argv_fuzz_flag)
        {
          ...
          // random generate argv 10 times for more sync opportunity
          for (int i = 0; i < 10; i++)
          {
            new_argv = (char **)ck_alloc(sizeof(char *) * 200);
            memset(new_argv, 0, sizeof(char *) * 200);
            memset(run_argv, 0, sizeof(run_argv));
            random_generate_arg(new_argv, argv, run_argv);
            fault = run_target(new_argv, exec_tmout);
            ...
            syncing_party = sd_ent->d_name;
            queued_imported += save_if_interesting(new_argv, mem, st.st_size, fault, run_argv);
...
int main(int argc, char **argv)
{
  ...
  while (1)
  {
    ...
    {
      if(do_arg_gen)queue_cur->arg_gen_parent->expect_new_cov_arg_gen_time = queue_cur->arg_gen_parent->total_arg_gen_exec_100us / queue_cur->arg_gen_parent->total_arg_gen_path;
      if(do_havoc)queue_cur->expect_new_cov_havoc_time = queue_cur->total_havoc_exec_100us / queue_cur->total_havoc_gen_path;
      if(do_splice)queue_cur->expect_new_cov_splice_time = queue_cur->total_splice_exec_100us / queue_cur->total_splice_gen_path;
      queue_cur = queue;
      ...
      if(do_arg_gen)
      {
        ...
        while (queue_cur)
        {
          if(!queue_cur->arg_was_fuzzed && queue_cur->arg_gen_parent==best_queue_cur->arg_gen_parent)
          {
            current_entry = number;
            break;
          }
