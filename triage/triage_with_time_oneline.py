...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        for entry in direntries:
            ...
            tokens[0] = 'ASAN_OPTIONS=detect_leaks=0 ASAN_OPTIONS=halt_on_error=0 ' + sys.argv[2]
            cmdline = ' '.join(tokens)
            print(entry.name[:10])
            os.system(cmdline + ' 2>&1 | grep AddressSanitizer')
            f.close()
            print((os.path.getctime(crash_dir+entry.name)-os.path.getctime(sys.argv[1]+'/queue_info'))/3600)
