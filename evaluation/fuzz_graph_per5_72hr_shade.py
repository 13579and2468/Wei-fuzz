...
def main():
    color_platte = ['blueviolet','orange','crimson','dimgray','cadetblue']
    ...
    colors = []
    for i in range(len(color_platte)):
        for j in range(5):
            colors.append(color_platte[i])
        
    c = 0
    shade_y_max = [0] * (3600 * 72 + 1)
    shade_y_min = [100000] * (3600 * 72 + 1)
    shade_y_min[0] = 0
    shade_x = list(range(3600*72 + 1))
    xs = [[],[],[],[],[]]
    ys = [[],[],[],[],[]]
    ...
    if len(sys.argv) < 2:
        ...
        for i in range(1, len(sys.argv)-1, 2):
            ...
            if c%5==4:
                ...
                plt.fill_between(shade_x[:3600*72][::10],shade_y_min[:3600*72][::10],shade_y_max[:3600*72][::10], facecolor=colors[c], alpha=0.13, interpolate=True)
